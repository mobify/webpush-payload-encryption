...
def encrypt_plaintext(key, nonce, plaintext):
    """
    Encrypts the given plaintext with the given AES key in GCM mode.

    In this context, GCM is Galois Counter Mode. See
    https://cryptography.io/en/latest/hazmat/primitives/symmetric-encryption/?highlight=cipher#cryptography.hazmat.primitives.ciphers.modes.GCM
...
def encode_auth_info(info_type, client_key, server_key):
    return '\0'.join((
        '%s: %s' % (HEADER_CONTENT_ENCODING, info_type),
        'P-256',
        (
...
def encrypt_encoded_payload(key, payload, auth=None):
    ...
    # This will throw ValueError if the key is invalid.
    ...
    ciphertext = encrypt_plaintext(aes_key, nonce, payload)
